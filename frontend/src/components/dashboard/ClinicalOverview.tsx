...
interface PatientData {
  id: string; name: string; age: number; sex: string; dx: string; status: 'crit' | 'stable' | 'review';
  screening?: any; glycemic?: any; vitals?: any; renal?: any;
}
...
export function ClinicalOverview({ user, roleLabel, patients, loading, error, onOpenPatient, onLogout, onAddPatient }: ClinicalOverviewProps) {
  ...
  const missingData = patients.filter(p => {
    const stringified = JSON.stringify({
      s: p.screening, g: p.glycemic, v: p.vitals, r: p.renal
    });
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 text-muted">
            ...
            <div className="font-mono text-xs uppercase tracking-widest text-gold animate-pulse">Syncing Clinical Registry</div>
            <div className="text-sm mt-2 opacity-60">Establishing secure connection to SHURA core...</div>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-32 text-muted">
            ...
            <div className="font-mono text-xs uppercase tracking-widest text-rose mb-2">Connection Failed</div>
            <div className="text-sm opacity-80 text-center max-w-sm">
              Unable to establish a secure connection to the SHURA backend. Please verify your network and ensure the API server is running.
            </div>
          </div>
        ) : patients.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-muted border border-dashed border-line rounded-lg mt-8">
            ...
            <div className="font-mono text-xs uppercase tracking-widest text-gold mb-2">Registry Empty</div>
            <div className="text-sm opacity-60 text-center max-w-sm">
              No patients are currently registered in the clinic roster.
            </div>
            <Button variant="outline" className="mt-6 border-gold/30 text-gold hover:bg-gold/10" onClick={onAddPatient}>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 flex flex-col">
              ...
              <IntelligenceOverview totalPatients={patients.length} criticalCount={criticalCount} reviewCount={reviewCount} />
              <ActivityFeed />
            </div>
            ...
            <div className="flex flex-col gap-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button variant="outline" className="h-20 flex-col gap-2 bg-void-2 border-line hover:border-gold/30 hover:bg-void-3" onClick={onAddPatient}>
                    <Plus className="w-5 h-5 text-gold" />
                    <span className="text-xs">New Intake</span>
                  </Button>
                  <Button variant="outline" className="h-20 flex-col gap-2 bg-void-2 border-line hover:border-gold/30 hover:bg-void-3">
