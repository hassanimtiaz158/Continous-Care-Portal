...
export function IntelligenceOverview({ totalPatients, criticalCount, reviewCount, patients = [] }: { totalPatients: number; criticalCount: number; reviewCount: number; patients?: any[] }) {
    ...
    <div className="flex flex-col gap-4 mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-void-2 border border-line p-4 h-64 flex flex-col">
          <h4 className="text-[10px] font-mono uppercase tracking-widest text-muted mb-4">Cohort Distribution</h4>
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={statusData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={2} stroke="none">
                  {statusData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.color} />
                  ))}
                </Pie>
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0a0a0a', border: '1px solid #262626', borderRadius: 0 }}
        ...
        <div className="bg-void-2 border border-line p-4 h-64 flex flex-col justify-center items-center text-center">
          <h4 className="text-[10px] font-mono uppercase tracking-widest text-muted mb-2">Board Processing Time</h4>
