...
export type WorkflowStage = 'intake' | 'evidence' | 'deliberation' | 'consensus' | 'review' | 'finalized';
...
export function CaseProgressTracker({ currentStage }: { currentStage: WorkflowStage }) {
  ...
  return (
    <div className="mt-8">
      <h4 className="text-[10px] uppercase tracking-widest text-gold font-mono mb-4">Case Progress</h4>
      <div className="flex flex-col gap-3 relative before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-[1px] before:bg-line">
        {STAGES.map((stage, i) => {
          ...
          return (
            <div key={stage.id} className={cn("flex items-center gap-3 relative z-10 transition-colors", isActive ? "opacity-100" : isCompleted ? "opacity-70" : "opacity-40")}>
              <div className={cn(
                "w-[15px] h-[15px] rounded-full flex items-center justify-center border bg-void transition-colors",
                isCompleted ? "border-teal text-teal" : isActive ? "border-gold bg-gold/10" : "border-line"
              )}>
                {isCompleted ? <Check className="w-2.5 h-2.5" /> : isActive ? <div className="w-1.5 h-1.5 rounded-full bg-gold" /> : null}
              </div>
              <span className={cn(
