...
export function ExportOverlay({ sessionId, onComplete }: { sessionId: string; onComplete: () => void }) {
        ...
        <div className="flex flex-col items-center gap-6 max-w-sm text-center">
          ...
          <div className="flex flex-col gap-2 h-16">
            <AnimatePresence mode="wait">
              {phase === 0 && (
                <motion.span key="0" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="font-mono text-sm tracking-widest uppercase text-cream">
                  Assembling Document...
                </motion.span>
              )}
              {phase === 1 && (
                <motion.span key="1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="font-mono text-sm tracking-widest uppercase text-cream">
                  Applying Cryptographic Seal...
                </motion.span>
              )}
              {phase === 2 && (
                <motion.span key="2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="font-mono text-sm tracking-widest uppercase text-teal">
                  Export Ready
                </motion.span>
              )}
            </AnimatePresence>
