...
export function WorkflowStageWrapper({
  title,
  status,
  children,
  summary,
}: WorkflowStageWrapperProps) {
    ...
    >
      ...
      <div
        className={cn(
          ...
          isPending ? "border-line" : isActive ? "border-gold" : "border-teal",
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {isLocked && (
          <span className="text-[10px] font-mono text-gold uppercase tracking-widest">Sealed</span>
        )}
      </div>
      ...
      >
        <div className="pb-4">{isActive || isLocked ? children : summary}</div>
      </motion.div>
    </div>
  );
}
