...
import { PatientData, BoardResult, WorkflowStage } from './types';
...
export function WorkspaceCanvas({
  patient, onFieldChange, boardResult, onRunBoard, isRunningBoard, 
  onApprove, onReject, isLocked, sessionId, chatMessages, onSendChat, currentStage,
  proveItMode, onToggleProveIt, hoveredMetric, onHoverMetric
  ...
  const getStatus = (stage: WorkflowStage) => {
    ...
    const stages: WorkflowStage[] = ['intake', 'evidence', 'deliberation', 'consensus', 'review', 'finalized'];
  ...
  return (
    <div className={`flex-1 h-full overflow-y-auto px-8 py-12 scroll-smooth relative transition-colors duration-500 ${proveItMode ? 'bg-void-3' : 'bg-void'}`}>
      ...
      <div className="absolute top-4 right-8 z-50">
        ...
        >
          <div className={`w-2 h-2 rounded-full ${proveItMode ? 'bg-teal animate-pulse' : 'bg-muted'}`} />
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <WorkflowStageWrapper 
          ...
          summary={<span className="text-xs text-muted font-mono">{Object.keys(patient.screening).length + Object.keys(patient.vitals).length} data points verified</span>}
        >
          <ClinicalEvidence 
            patient={patient} 
        ...
        <WorkflowStageWrapper 
          ...
          summary={<span className="text-xs text-muted font-mono">{deliberationComplete ? 'Specialist board deliberation complete' : 'Awaiting deliberation'}</span>}
        >
          <AIBoardSection 
        ...
        {isLocked && (
          <div className="mt-16 pt-16 border-t border-line">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <ClinicalDiscussion sessionId={sessionId} messages={chatMessages} onSend={onSendChat} />
            </div>
          </div>
        )}
